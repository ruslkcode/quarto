...
public class SmartStrategy implements BotStrategy{
    ...
    public Move determineMove(Game game) {
        ...
        // Safety fallback
        if (moves.isEmpty()) {
            return getValidMoves(game).get(0);
        }
    ...
    public Move findImmediateWin (Game game) {
        ArrayList<Move> allmoves = new ArrayList<>(getValidMoves(game));
        for (Move move : allmoves) {
            int current = game.getCurrentPlayer();
            Game copy = game.deepCopy();
            copy.doMove(move);
            if (copy.getWinner() == current) {
                return move;
            }
        }
        return null;
    ...
    private List<Move> filterImmediateLoss(Game game, List<Move> moves) {
        ...
        for (Move move : moves) {
            Game copy = game.deepCopy();
            copy.doMove(move);
            boolean opCanWin = false;
            for (int field : getEmptyFields(copy)) {
                Game simulation = copy.deepCopy();
                int dummyNextPiece = getAnyAvailablePiece(simulation);
                simulation.doMove(new Move(dummyNextPiece, field));
        ...
        for (Move move: moves){
            ...
            for (int field : getEmptyFields(copy)) {
                ...
                for (int piece : copy.getAvailablePieces().keySet()) {
                    ...
                    if (simulation.getWinner() == 0) {
                        losingForAllPieces = false;
                        break;
                    }
                }
                if (losingForAllPieces) {
                    unavoidable = true;
                    break;
                }
            }
    ...
    private boolean opponentHasFork(Game game, List<int[]> lines) {
        ...
        for (int field = 0; field < 16; field++) {
            if (game.getBoard().isEmptyField(field)) {
                if (isForkField(game, field, lines)) {
                    return true;
                }
            }
        }
    ...
    private int countSafeMoves(Game game, List<int[]> lines) {
        List<Move> moves = getValidMoves(game);
        // apply existing filters in the correct order
        moves = filterImmediateLoss(game, moves);
        moves = filterUnavoidableLoss(game, moves);
        moves = filterAntiFork(game, moves, lines);
        return moves.size();
    ...
    private int mobilityScore(Game game, Move move, List<int[]> lines) {
        ...
        Game afterMyMove = game.deepCopy();
        afterMyMove.doMove(move);

        // opponent mobility
        int opponentMoves = countSafeMoves(afterMyMove, lines);

        // simulate opponent passing a piece back to us
        // (we only care about our potential freedom, not exact move)
        int myMoves = 0;
        for (int pieceId : afterMyMove.getAvailablePieces().keySet()) {
            Game sim = afterMyMove.deepCopy();
            sim.doMove(new Move(pieceId)); // opponent gives us a piece
            myMoves += countSafeMoves(sim, lines);
        }

        // average future freedom
        if (!afterMyMove.getAvailablePieces().isEmpty()) {
            myMoves /= afterMyMove.getAvailablePieces().size();
        }
